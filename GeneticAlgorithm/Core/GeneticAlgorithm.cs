...
public class GeneticAlgorithm : IGeneticAlgorithm
{
	...
	protected Individual NewGeneration()
	{
		...
		//------ 生存を行う。優秀な親は次世代集団に残る ------//
		List<Individual> survivors = model.SurvivalAlgorithm.Survive(group);

		//------ 淘汰を行う。優秀な個体が多く残る ------//
		List<Individual> new_group = model.SelectionAlgorithm.Select(group);

		//------ 交叉を行う。生存しなかった親は全て入れ替える ------//
		List<Individual> children = model.CrossoverAlgorithm.Crossover(new_group, new_group.Count - survivors.Count);
	...
	public void Reset()
	{
		...
		{
			//------ 個体を準に生成 ------//
			group.Add(model.IndividualModel.CreateNewIndividual());
		}
	}
