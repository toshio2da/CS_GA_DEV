...
namespace GALib.Algorithm
{
	...
	public class GeneticAlgorithm : IGeneticAlgorithm
	{
		...
		public GASearchResult Search(GASearchContext context)
		{
			...
			ret.GenerationCount = 0;
			try
			{
				//------ 各世代毎の優秀な個体を登録していく ------//
				//List bestIndividuals = new LinkedList();                             // 各世代の最優秀者の一覧
				startingTime = DateTime.Now.Ticks;                     // 現在の時刻を記録
				...
				foreach (var individual in group)
				{
					...
					double fitnessValue = model.FitnessAlgorithm.GetFitnessValue(individual);   // 適応度を計算
					if (fitnessValue == model.FitnessAlgorithm.BestFitnessValue)
					{
						// 究極の個体か検証
				...
				this.context.SetBestIndividual(group[0]);


				//------ 最大世代交代数まで繰り返す ------//
				nowGenerationNumber = 0;
				...
				for (int index = 0; index < this.context.MaxGenerationCnt; index++)
				{
					...
					ret.GenerationCount = index + 1;

					//------ 世代毎の一番優秀な個体を登録していく ------//
					this.context.SetBestIndividual(NewGeneration());
					...
					bestIndividual.GenerationNumber = nowGenerationNumber;
					...
					else
					{
						...
						if (StopSearchGeneration > 0 && lastEvolutionGeneration > StopSearchGeneration)
						{
							break;
						}
						else if (StopSearchInterval > 0 && (DateTime.Now.Ticks - lastEvolutionTime) / 1000 > StopSearchInterval)
						{
							break;
						}
					}
				...
				this.context.SearchStatusType = GASearchStatus.DONE_SEARCH;
				this.context.Reporter.FinishReport(lastSuperior, nowGenerationNumber, DateTime.Now.Ticks - startingTime);
			...
			catch (Exception ex)
			{
				...
				throw new GeneticAlgorithmException(ex);
			}
			finally
			{
		...
		protected Individual NewGeneration()
		{
			...
			if (context.GetBestIndividual() == null) { }
			else if (context.GetBestIndividual().FitnessValue == model.FitnessAlgorithm.BestFitnessValue)
			{
				return context.GetBestIndividual();
			}

			//------ 生存を行う。優秀な親は次世代集団に残る ------//
			List<Individual> survivors = model.SurviveAlgorithm.Survive(group);

			//------ 淘汰を行う。優秀な個体が多く残る ------//
			List<Individual> new_group = model.SelectionAlgorithm.Selection(group);

			//------ 交叉を行う。生存しなかった親は全て入れ替える ------//
			List<Individual> children = model.CrossoverAlgorithm.Crossover(model.IndividualFactory, new_group, new_group.Count - survivors.Count);

			//------ 突然変異を子集団の各塩基に対して行う。突然変異率が0.0の場合は行わない ------//
			double mutationProbability = model.MutationProbability;
			...
			double inverseProbability = model.InverseProbability;
			...
			group = group.OrderByDescending(e => e.FitnessValue).ToList();// 適応度でソート
			Individual bestIndividual = group[0];                    // 世代の中で一番優秀な個体を取得
		...
		public void Reset()
		{
			...
			group.Clear();
			//------ 今までの記録も削除 ------//
			context.ClearSuperior();

			//------ 集団を再形成 ------//
			for (int index = 0; index < context.IndividualCnt; index++)
			{
				//------ 個体を準に生成 ------//
				group.Add(model.IndividualFactory.CreateNewIndividual());
			}
		}
		...
		{ //throws OutOfBoundsGeneException {
			...
			{
				//------ 個体を遺伝子を取得 ------//
				Individual individual = individualIterator.Current;  // 突然変異を行う個体を取得
		...
		{ //throws OutOfBoundsGeneException {
			...
			while (individualIterator.MoveNext())
			{
				...
				//------ 個体と遺伝子を取得 ------//
				Individual individual = individualIterator.Current;
